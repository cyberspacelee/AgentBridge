...
浏览器访问 `http://127.0.0.1:3000/agents`。Pi 0.85.1、OpenCode 1.18.29 和 Pi MCP 扩展 2.32.1 随锁文件安装。Codex、Grok 使用主机现有 CLI，通过 `CODEX_COMMAND`、`GROK_COMMAND` 指定命令或可执行文件绝对路径。本次原生验证版本为 Codex 0.153.4、Grok 1.0.5；其他版本必须确认协议能力。运行时下载安装与版本切换尚未实现，设计见 [版本管理方案](docs/design/RUNTIME_INSTALL.md)。
...
Skills 登记包含 `SKILL.md` 的本地绝对目录，保留原文件和附件。MCP 支持 stdio 命令数组、环境变量以及远程 HTTP URL、请求头。只向选中的 Agent 分配已启用资源；Pi 使用随项目安装的 MCP 扩展，不提供任意插件安装页面。
...
一个任务对应一个 Session，连续对话默认展示完整历史。每次提交创建 Run，承担排队、超时、取消、用量与交付物归属；没有额外 Turn 实体。默认沿用上次仍可用的模型，否则使用当前 Agent 默认模型。每个 Run 记录实际配置修订。无法取得的 token 或费用显示未知。

## 启动参数与环境

启动器只读取启动工作目录的 `.env`，已有进程环境变量优先。CLI 的 host/port/engine 参数覆盖对应环境变量。业务配置保存后，以 settings.json 为准。

| 配置 | 范围 |
| --- | --- |
| `AGENT_HOST` / `AGENT_PORT` | 默认 127.0.0.1 / 3000 |
| `AGENT_DATA_DIR` | 默认当前目录下 .agentbridge，多个实例必须分开 |
