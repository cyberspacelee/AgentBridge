...
stdout 和 `.agentbridge/logs/` 输出 JSON 日志，`time` 使用系统默认时区的 ISO 8601 格式，包含毫秒和时区偏移（例如 `2026-09-07T16:30:00.123+08:00`）；设置 `TZ` 环境变量时遵循该配置。文件按 10 MiB/天轮转，保留当前文件及 5 个历史文件。SQLite 错误日志最多保留 10,000 条。正文和密钥不进入常规请求日志。
