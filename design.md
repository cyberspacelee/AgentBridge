...
| 宽度 | 导航 | 主内容 | 任务信息 |
| --- | --- | --- | --- |
| ≥ 1280 px | 208 px 侧栏 | 24 px 页边距，最大 1500 px | 280 px 右栏，可收起 |
...
桌面执行页使用可用视口高度，消息区域 `min-height: 0; overflow-y: auto`，输入区作为同一 flex 列的固定兄弟元素，不覆盖最后一条消息。窄屏允许页面纵向流动，输入区保持可达并适配软键盘，不机械沿用 `65svh` 嵌套滚动。
