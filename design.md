...
版本：1.2 · 2026-09-08。状态：统一配置与工作台交互规范；功能回归与视觉验收分别记录，不以自动测试通过替代体验判断。
...
| 页面 | 首要判断 | 首屏内容 | 次级内容 |
| --- | --- | --- | --- |
| 任务工作台 | 哪些任务需要处理 | 可执行入口、状态筛选、任务列表 | 目录、执行记录 |
...
统一壳层：左侧导航、顶部上下文、主内容。主入口为任务工作台、Agent 管理、共享资源、运行观测；系统信息为辅助入口。
...
主题提供浅色、深色、跟随系统三个选项，沿用 ThemeProvider 存储。新任务可以选择已启用 Agent，已有任务不允许更换引擎。
...
默认展示完整对话，执行筛选仅在用户选择后限制 Message 范围，并保持消息和 parts 的服务端顺序。用户要求用窄幅中性背景；Agent 回答用无外框正文。角色、时间与复制操作共用统一消息头，避免每条消息配装饰头像。
