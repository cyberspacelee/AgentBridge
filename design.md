...
版本：1.5 · 2026-09-09。采用稳定导航、独立历史页和统一会话输入面；Web 与 Electron 共用前端。历史 1.4 验收不能替代本轮布局验收。历史截图仅对应其记录日期。
...
- 配置状态为未保存、已保存待应用、应用中、已生效或失败。使用统一操作栏集中保存、保存并应用／启用、撤销；异步请求受理不显示完成提示，最终状态来自 Agent 快照。
...
设置操作栏位于所属表单或设置区末尾，采用完整 1 px 边框、card 表面、8 px 圆角、桌面 16 px / 窄屏 12 px 内边距。状态在左、操作在右，窄屏换行；不跨设置区吸底悬浮，不覆盖字段、卡片边框或键盘焦点。网络设置与 Agent 配置共用 `.config-action-bar`；单卡片的动作沿用 CardFooter。

## 5. 应用布局

统一壳层：单一左侧栏、顶部上下文、连续主内容。主入口为新会话、历史会话、Agent 管理、共享资源、运行观测；系统信息为辅助入口。侧栏不展示会话记录。历史以独立内容页承载搜索、筛选和分页。

| 宽度 | 导航 | 主内容 | 任务信息 |
| --- | --- | --- | --- |
| ≥ 1280 px | 248 px 统一侧栏，可收为 64 px | 24 px 页边距，管理页最大 1500 px，对话列最大 800 px | 280 px 右栏，默认收起；正文不足 720 px 时使用弹层 |
...
侧栏从上到下为品牌与折叠按钮、新会话与历史入口、管理入口、底部系统信息。导航在不同页面保持一致，按入口职责用间距分组。展开时导航为图标与文字，折叠时为具名图标与 Tooltip；折叠操作保留 aria-expanded 和布局偏好。窄屏只用一个导航 Sheet，打开时不自动聚焦搜索或唤起键盘。

历史页 `/conversations/history` 使用主内容区域，最大 1200 px。名称和工作目录占主列，Agent、状态、更新时间为次级列；窄屏重排为紧凑两行，禁止横向裁切。标题最多两行，路径省略并可通过 title 或详情查看。列表按现有 API 的创建时间倒序排列，更新时间单独展示，不用不一致的日期分组暗示排序。搜索范围是全部会话的名称与 ID，不能标为全文搜索。每页默认 20 条，使用服务端游标和明确的上一页、下一页；直接打开未知游标时提供返回第一页。打开详情再返回保留 URL 筛选、游标及当前应用内的列表滚动位置；刷新不承诺保留滚动。列表最小可用高度 160 px，低高度时内容页允许滚动，不能让列表消失。先用有界分页，不为当前 20 行引入虚拟列表。

主题提供浅色、深色、跟随系统三个选项，沿用 ThemeProvider 存储。新会话欢迎标题桌面 30 / 40 px、窄屏 24 / 32 px，其余页面沿用普通标题尺寸。新会话 `/conversations` 以居中标题和 720 px 组合输入面为视觉中心；输入面内放消息、模型摘要和发送按钮，下接 Agent、目录和高级设置，避免排列成多个同等强调的大表单区。默认选择就绪的默认 Agent，也可选择其他就绪 Agent；已创建会话不允许更换引擎。必填工作目录在首次提交前始终可发现，标题、模型与策略放在“模型与会话设置”。Header 不提供与当前内容无关的刷新。没有就绪 Agent 时，整个输入区替换为一组居中引导：Agent 图标、连接你的第一个 Agent、说明、独立配置按钮及历史入口；禁止标题左对齐、空状态居中，禁止将按钮嵌入 EmptyTitle。首次配置与正常输入采用同一内容中心。

任务详情的空间顺序：

```text
稳定侧栏（248 px） | 当前会话标题 / 状态 / 必要操作
品牌与折叠按钮    | 对话 / 交付物 / 交互 / 诊断 · 执行筛选
新会话 / 历史会话 | 连续正文（最大 800 px）       | 按需任务信息
Agent / 资源 / 观测| 用户要求 / Agent 回答        | 引擎 / 模型
                  | 工具摘要 -> 展开结果与输入   | 工作目录
                  | 待处理交互 -> 就地回复       | 用量 / 时间
系统信息          | 追加输入 / 模型上下文 / 发送  |
```
...
本轮验收覆盖稳定导航、首次配置与可输入两种首页、独立历史页的搜索与分页、详情返回位置、1280×560 与窄屏、浅深色和旧路由重定向。前端统一 `/conversations`、`/conversations/history`、`/conversations/:id`；`/tasks` 旧链接仅重定向并保留查询参数与锚点，含历史筛选的旧首页进入历史页。HTTP `/api/tasks` 仍为既有 TaskSummary 契约，前端路由改名不改变 API 或数据库格式。Electron 内部导航白名单同时允许新路由和旧重定向入口。历史截图不证明本版实现；真实引擎、Windows/macOS 原生窗口和屏幕阅读器需分别记录实际验收范围。
