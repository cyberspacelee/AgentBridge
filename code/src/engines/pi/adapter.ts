  ...
  private async openSession(session: Session, expectedNativeId?: string) {
    ...
    const child = startProcess(
      ...
      session.directory,
      {
        ...
        PI_TELEMETRY: "0",
        ...(settings.mcp.length ? { AGENT_BRIDGE_PI_MCP_MODULE: createRequire(realpathSync(resolveExecutable(this.config.pi.command) ?? this.config.pi.command)).resolve("pi-mcp-adapter") } : {}),
