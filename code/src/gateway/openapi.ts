...
const schemas: Record<string, Schema> = {
  ...
  RuntimeInfo: object({ instanceId: str, storeId: str, engine: str, health: ref("EngineHealth"),
    ...
    storage: enumeration("sqlite", "memory"), models: array(ref("Model")), limits: dictionary(num), interactionDefaults: ref("InteractionPolicy"), capabilities: dictionary(bool), llmProxy: object({ ready: bool, baseUrl: nullable(str), enabledProviders: int }) }),
...
operation("post", "/api/providers/{id}/test", "testProvider", "Agent 与资源", "测试已保存模型连接", object({ ok: { type: "boolean", const: true }, durationMs: num, modelID: str }), { input: object({ modelID: { type: "string", minLength: 1, maxLength: 300 } }), errors: [502], description: "id 为配置中的 provider ID；通过已保存 effective route 发起真实模型请求，覆盖协议转换。失败返回 502 MODEL_CONNECTION_ERROR，耗时单位毫秒。" });
...
operation("get", "/api/observability/llm", "listLlmRequests", "观测", "查询 LLM 请求记录", object({ items: array(object({ eventId: str, occurredAt: date, providerID: nullable(str), modelID: nullable(str), clientApi: str, upstreamApi: str, conversion: str, status: int, durationMs: num, ttftMs: nullable(num), stream: bool, usage: nullable(object({ input: nullable(num), output: nullable(num), cacheRead: nullable(num), cacheWrite: nullable(num), costUsd: nullable(num) })), error: nullable(str) })), from: date, to: date }), { parameters: [...timeRange, parameter("provider", { type: "string", default: "" }, "Provider ID。"), parameter("model", { type: "string", default: "" }, "模型 ID。"), parameter("protocol", { type: "string", default: "" }, "client protocol。"), parameter("conversion", { type: "string", default: "" }, "转换模式。"), parameter("limit", { type: "integer", minimum: 1, maximum: 100, default: 50 }, "最多返回条数。") ] });
