...
主导航：新会话、历史会话、Agent 管理、共享资源、运行观测；系统信息位于侧栏底部。历史会话在独立页面检索和分页，不嵌入侧栏。Agents 列表展示开关、健康、默认模型、活动数量及应用状态；详情以运行、模型、Skills、MCP 分页编辑，可测试模型和预览原生导入。共享资源在同一管理模块维护；系统设置显示网关目录、限制与连接状态。复用已有主题、Base UI、表格与表单，不引入新的视觉系统。

会话详情页默认展示整个 Session 的连续对话，各 Run 有状态和耗时分隔；执行历史筛选供诊断使用。交互就地可回复，交付物和诊断按执行筛选。新建会话选择启用且就绪 Agent，默认模型与策略由 Agent 提供。
