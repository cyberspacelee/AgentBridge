...
- 一个 HTTP 网关，一套标准会话、消息、事件和任务执行流程，四个真实引擎适配器。
- 新增 Agent 只增加适配实现、启动注册、引擎配置和契约验证，不修改现有路由、会话执行逻辑或其他引擎。
- 提供一个前端应用，包含任务工作台、Agents 管理、网关观测和系统设置；任务详情归属工作台，支持分派、交互和查看产物，观测模块覆盖整体健康、性能与故障。
...
Electron 主进程管理窗口、托盘和 utility process 网关；现有 Fastify/SQLite/SessionRuntime 在该进程运行，React 继续使用同源 HTTP/SSE。Web/Desktop 通过共用 Supervisor 持久管理监听 IP 与端口，默认 127.0.0.1:3000，可开放局域网；HTTP/SSE 和 Web 无需配对或鉴权。renderer 不获得 Node 或任意 IPC 权限。关闭窗口继续后台运行，显式退出复用任务等待/取消和引擎清理。

默认桌面包含 Electron 自带网关运行时，不含独立 Node/npm 和四个 Agent CLI；管理页按需安装官方最新版、更新和卸载，程序与用户状态分目录保存。安装受管 Agent 时才按需下载 Node/npm toolchain。安装切换在 Agent 生命周期入口串行执行，停止原生进程并备份原生目录/会话绑定后切换，失败恢复旧版，卸载保留历史与配置。应用构建依赖固定，CLI 版本独立更新。Web 和 Desktop 共用 `host/Supervisor`、网络策略和逐 Agent CLI 来源。Desktop 仅负责窗口、托盘、系统对话框、偏好和应用更新；业务页面通过同一 HTTP/SSE API。详见 [统一运行](docs/design/WEB_DESKTOP.md)。
...
图中适配器是同一网关内独立管理的实现；Pi、OpenCode、Codex、Grok 均注册，只有已启用的 Agent 启动。状态存储与事件发布是执行层内部模块，不是独立服务。
...
依赖约束：前端只调用本网关；路由只调用执行层；执行层只依赖适配契约；适配器可以使用进程管理模块；工具不反向依赖网关。适配器注册位于启动装配文件；运行时按 Session.engineId 路由，启停由统一生命周期管理。共享类型只定义引擎无关的数据，不导入任何原生 SDK 或 HTTP 框架。
...
3. 在 `main.ts` 的静态名称到构造函数映射中注册一个入口。首次配置的默认 Agent 来自 `--engine > AGENT_ENGINE > opencode`；持久化后以 settings.json 的 defaultAgent 为准，非法名称报错。
4. 添加本引擎配置和启动文档；把公共工具目录、Python 路径、工具说明接入其原生配置方式。
5. 运行同一份契约与生命周期测试，再做实际样本验收。

装配入口只保留普通映射，例如 `opencode -> createOpenCodeAdapter`、`pi -> createPiAdapter`。无需扫描适配器目录、反射注册或多层 Factory 类；CLI 的受管安装与版本更新独立于适配器注册；增加一个映射项是允许的扩展成本。
...
- 每个网关会话启动一个 `pi --mode rpc --session <受管会话文件>` 子进程，cwd 保持任务工作目录；原生历史落盘用于恢复上下文。网关业务记录与原生上下文分别持久化，历史 Run 不重放。
...
各 Agent 通过所选 Skills、MCP 及原生工具提供能力，网关不内置 Office/Python 业务脚本。共享资源定义在 settings.json，每个 Agent 引用所需项目。
